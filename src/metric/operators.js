...
const apiToString = state => ({
    toString: ()=> {
        const {left, name, right} = state;
...
const apiValueAt = state => ({
    valueAt: i => {
        const { operation, left, right } = state;
        return operation(left.valueAt(i), right.valueAt(i));
    }
});

const apiShift = state => ({
    shift: offset => {
        shift
    }
})

const operator = (name, operation) => {
    const state = {
        name,
        operation,
        left: 0,
    ...
    return Object.assign(state, apiToString(state), apiValueAt(state))
}
